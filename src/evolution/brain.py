"""THE BRAIN: LAB-only strategy mission orchestrator.

Turns an immutable baseline strategy into a structured mission for specialist agents.
No broker connectivity and no live trading are allowed here.
"""
...
def _active_agents(state):
    return [a for a in state.get("agents", []) if str(a.get("status", "")).lower() != "retired"]

def create_strategy_mission(strategy_id: str, baseline_name: str, agents: Iterable[dict] | None = None) -> dict:
    s = load_state()
    pool = list(agents) if agents is not None else _active_agents(s)
    mission = Mission(f"MISSION-{len(s.get('missions', []))+1:05d}", strategy_id, baseline_name, datetime.now(timezone.utc).isoformat())
    s.setdefault("missions", []).append(asdict(mission))
    tasks=[]; cursor=0
    for team, requested, objective in TEAM_PLAN:
        for _ in range(requested):
            if not pool: break
            agent=pool[cursor % len(pool)]; cursor += 1
            aid=agent.get("agent_id") or agent.get("id") or f"AGENT-{cursor:04d}"
    ...
    append_event(s,"brain_mission_created",{"mission_id":mission.id,"strategy_id":strategy_id,"baseline":baseline_name,"tasks":len(tasks)})
    s["live_trading_enabled"] = False
    save_state(s)
    return {"mission":asdict(mission),"tasks":tasks,"teams":TEAM_PLAN}
...
def team_summary(state=None):
    ...
    for t in s.get("tasks",[]):
        team=t.get("team"); status=t.get("status","queued")
        if team in out: out[team][status]=out[team].get(status,0)+1
    return out
...
def promote_only_after_gates(candidate: dict, gates: dict) -> bool:
    ...
    return all(bool(gates.get(k)) for k in required) and candidate.get("baseline") is not True
